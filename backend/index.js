...
app.get('/meals/:userEmail', async (req, res) => {
  try {
    ...
    const meals = await Meal.find({ userEmail }); // Fetch meals associated with the user
...
app.post('/add-trainer', async (req, res) => {
  ...
  try {
    ...
    const newTrainer = new trainers({ name,email, phone,  address, password });
...
// Route to fetch all meals for a specific user using query parameter
